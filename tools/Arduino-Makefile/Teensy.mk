...
ifeq (, $(shell which wslpath))
	...
	RESET_CMD = nohup $(ARDUINO_DIR)/hardware/tools/teensy_post_compile.exe -board=$(BOARD_TAG) -tools="$(shell wslpath -w $(abspath $(ARDUINO_DIR)/hardware/tools))" -path="$(shell wslpath -w $(abspath $(OBJDIR)))" -file=$(TARGET) > /dev/null ; $(ARDUINO_DIR)/hardware/tools/teensy_reboot.exe
endif
